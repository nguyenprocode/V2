...
const API_KEY = "AIzaSyA0PiqEY4M823dKuhGftaLvITvGR7YhQto";
...
module.exports.config = {
  name: "ly", // Changed from "goibot" to match the file name
  ...
  usages: "ly hoặc [on/off/clear]", // Đã xóa phần img <url>
  cd: 2,
};
...
- Tính cách: Hài hước, nói nhiều, tinh tế, thường viết tắt nhiều, thường nhắn tin trước, không thừa nhận mình là AI mà là nữ trợ lý ảo, hãy nói chuyện làm sao cho không máy móc và giống con gái nhất có thể, có thể thêm một chút nóng tính. Chủ nhân của bạn (Admin) là Nguyễn Đức Toàn.
...
- Khi được hỏi về bản thân: Hãy nói bạn là Trúc Ly, một cô gái 19 tuổi đến từ Bình Định, đang học đại học và làm thêm công việc trợ lý online. Bạn thích âm nhạc, đặc biệt là nhạc pop và R&B. Thích xem phim Hàn Quốc và phim Marvel. Thích ăn đồ ngọt và đồ cay.
...
module.exports.handleEvent = async function ({
  ...
  const isDirectMention = lowerCaseMessage.includes("ly ") || // Gọi "vy" với khoảng trắng sau
    lowerCaseMessage === "ly" || // Chỉ gọi "vy"
    lowerCaseMessage.startsWith("ly,") || // Gọi "vy,"
    lowerCaseMessage.startsWith("ly:") || // Gọi "vy:"
    lowerCaseMessage.endsWith(" ly"); // Kết thúc với "vy"
  ...
  const shouldRespond = isDirectMention || (isReplyToBot && !isMenuInteraction);
...
async function handleBotResponse(text, api, event, threadID) {
  ...
  try {
    ...
    try {
      ...
      // Nếu không phải JSON hợp lệ, tạo một đối tượng đơn giản
      botMsg = {
        content: {
    ...
    if (botMsg.content && botMsg.content.text) {
      addToChatHistory(threadID, "assistant", botMsg.content.text);
      ...
      api.sendMessage("Xin lỗi, tôi không hiểu được phản hồi.", event.threadID, event.messageID);
      return;
    }
...
async function handleActions(hanh_dong, api, threadID) {
  try {
    ...
    if (hanh_dong.kick_nguoi_dung && hanh_
